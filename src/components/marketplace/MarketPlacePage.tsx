...
const HeroSection: FC = () => (
    <div className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-purple-800 to-pink-800 text-white">
        ...
        <div className="absolute inset-0">
            <div
                className="absolute top-10 left-10 w-72 h-72 bg-purple-400/20 rounded-full blur-3xl animate-pulse"></div>
            <div
                className="absolute bottom-10 right-10 w-96 h-96 bg-pink-400/20 rounded-full blur-3xl animate-pulse delay-1000"></div>
        </div>
        <div className="relative px-6 py-20 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button
                    className="px-8 py-4 bg-gradient-to-r from-yellow-400 to-orange-500 text-black font-bold rounded-2xl hover:scale-105 transform transition-all duration-300 flex items-center justify-center shadow-2xl">
                    <Zap className="w-5 h-5 mr-2"/>
                    Explorar Agora
                </button>
                <button
                    className="px-8 py-4 bg-white/10 backdrop-blur-sm border border-white/20 text-white font-semibold rounded-2xl hover:bg-white/20 transition-all duration-300 flex items-center justify-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                    {stats.map((stat, i) => (
                        <div key={i}
                            ...
                            <div
                                className={`inline-flex p-4 rounded-2xl ${stat.color} bg-gray-50 mb-3 group-hover:bg-gray-100`}>
...
const ProductCard: FC<ProductCardProps> = ({product, onLike, onShare, onAddToCart, key}) => {
    ...
    if (product.images) {
        if (product.images.data) {
            const imageUrl = product.images?.data?.[0]?.attributes?.url
                ? `${STRAPI_URL}${product.images.data[0].attributes.url}`
    ...
    return (
        <div
            className="bg-white rounded-3xl shadow-lg hover:shadow-2xl border border-gray-100 overflow-hidden group transition-all duration-500 hover:scale-105 transform"
        ...
        >
            <div className="relative aspect-square overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                    {product.isFeatured && (
                        <div
                            className="flex items-center px-3 py-1.5 bg-gradient-to-r from-yellow-400 to-orange-500 text-black text-xs font-black rounded-full">
                    ...
                    {product.isOnSale && (
                        <div
                            className="flex items-center px-3 py-1.5 bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs font-black rounded-full">
                ...
                <div
                    className="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </div>
            ...
            <div className="p-6">
                ...
                <div className="flex items-center mb-3">
                    <div
                        className="w-6 h-6 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 flex items-center justify-center text-white text-xs font-bold mr-2">
...
const MarketplacePage = () => {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await productsAPI.getProducts(page, 12, selectedCategory === 'all' ? undefined : selectedCategory);
        ...
        <div className="space-y-8">
            ...
            <div className="max-w-7xl mx-auto px-6 space-y-6">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center text-red-500 bg-red-50 p-6 rounded-2xl border border-red-200">
                        {error}
                    </div>
                ) : products.length === 0 ? (
                    <div className="text-center text-gray-500 bg-white p-10 rounded-2xl border">
                    ...
                    <div className="flex justify-center pt-8">
                        <div className="flex items-center space-x-2">
                            <button
                                ...
                                disabled={page === 1}
                                className="px-4 py-2 border rounded-xl disabled:opacity-50 flex items-center gap-2"
                            >
